...
UPSTREAM_URL="https://github.com/nextcloud/android.git"

usage() {
  cat <<'USAGE'
Usage: scripts/mynextcloud/sync_upstream_android.sh /path/to/myNexcloud-for-android [--apply]
...
main() {
  ...
  git -C "$repo_dir" status --short
  echo "Upstream wurde gefetcht. Bitte Branding-, Package- und Store-Diff pruefen, bevor gemergt wird."
}
