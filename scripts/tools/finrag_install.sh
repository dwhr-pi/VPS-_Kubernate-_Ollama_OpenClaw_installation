...
ensure_sudo_ready_for_long_build() {
  ...
  log_finrag "Pruefe sudo-Berechtigung vor dem langen CPython-Build." >&2
  log_finrag "Hinweis: Wenn das Passwort falsch eingegeben wird, bricht der Installer jetzt sofort ab und nicht erst nach dem Build." >&2
  if ! sudo -v; then
    log_finrag "Fehler: sudo-Authentifizierung fehlgeschlagen. Bitte Tastaturlayout/CapsLock pruefen und erneut starten." >&2
    return 1
  fi
}
